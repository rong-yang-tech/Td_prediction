...
# # KRR回归
    ...
    krr = GridSearchCV(KernelRidge(kernel='rbf'),scoring=None,
                      param_grid={'alpha': np.logspace(-3, 0, 10),  # 15个对数间隔值
                                    'gamma': np.logspace(-3,0, 10)} ,  # 基于数据尺度的gamma,
...
#     new_pre_smi = pd.read_csv(new_csv)
    ...
    # mae为基准
    mae_list = [(MAE(y_test, y_pre_test_gbr),gbr_new_smi), (MAE(y_test, y_pre_test_krr),krr_new_smi ),(MAE(y_test, y_pre_test_rfr),rfr_new_smi),(r2_score(y_test,  stacking_prediction),stacking_new_smi)]
...
if __name__ == '__main__':
    ...
    for i in range(1,2):
        ...
        csv=pd.read_csv("C:/Users/888/Desktop/smi_new_rdk_des3.csv")
