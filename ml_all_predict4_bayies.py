...
def rfr_cv(n_estimators, min_samples_split, max_features, max_depth):
    val = cross_val_score(
        RandomForestRegressor(n_estimators=int(n_estimators),
                              ...
                              random_state=2,),
        x_train, y_train, scoring='neg_mean_absolute_error', cv=10, error_score='raise',n_jobs=-1,
    ).mean()
    return val
...
df=pd.read_csv("./ml_data13_rdk000.csv")
# df = pd.read_csv('D:/smi_xyz/smil_to_xyz/dft_properties/new_useful_data/new_useful_data/model decom/clean_bde.csv')# 效果最佳，mae=22.1
#df = pd.read_csv('D:/smi_xyz/smil_to_xyz/dft_properties/new_useful_data/new_useful_data/model decom/clean_del0_bde.csv')# 效果最佳，mae=22.1
# df = pd.read_csv("D:/smi_xyz/smil_to_xyz/dft_properties/new_useful_data/new_useful_data/model decom/crystal_interacrion1/inp/clean_del0_bde_inter3.csv")
# df = pd.read_excel('all1.xlsx')
# df = pd.read_csv("D:/smi_xyz/smil_to_xyz/dft_properties/new_useful_data/new_useful_data/model decom/dft_rdk_crystal_descriptor/all_new_dis_ele.csv")
# y = df['DECOMPOSITION[℃]']
y = df['T']
#x = df.loc[:, 'E_HOMO(N+1)(eV)':'ESTATE_60']
# x = df.loc[:, 'E_HOMO(N+1)(eV)':'NPR2']
# x = df.loc[:, 'bde':'mean_Ellipticity_of_electron_density']
x = df.drop(['T', 'can_smiles'], axis=1)
# 特征相关性分析
...
x_cor = x_cor.abs()
print(x_cor[y.name])
drop_col=x_cor[x_cor[y.name]<0.01].index
print(drop_col)
...
opt(pbounds_rfr,rfr_cv)
opt(pbounds_krr,krr_cv)
opt(pbounds_gbr,gbr_cv)
opt(pbounds_mlp,mlp_cv)

df_pre_new=pd.read_csv("./pre_smi_new_rdk_des3.csv")
y_pre_new = df_pre_new['T']
#x = df.loc[:, 'E_HOMO(N+1)(eV)':'ESTATE_60']
# x = df.loc[:, 'E_HOMO(N+1)(eV)':'NPR2']
# x = df.loc[:, 'bde':'mean_Ellipticity_of_electron_density']
...
rfr = RandomForestRegressor(max_depth=int(params_best_rfr['max_depth']),
                            max_features=float(params_best_rfr['max_features']),
                            min_samples_split=int(params_best_rfr['min_samples_split']),
                            n_estimators=int(params_best_rfr['n_estimators']), random_state=2)
model_fit(rfr)
...
gbr = ensemble.GradientBoostingRegressor(n_estimators=int(params_best_gbr['n_estimators']),
                                         ...
                                         max_depth=int(params_best_gbr['max_depth']),
                                         subsample=params_best_gbr['subsample'], alpha=params_best_gbr['alpha'],
                                         loss='huber',
                                         random_state=2)
model_fit(gbr)
